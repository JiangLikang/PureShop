	...
	new Vue({
		...
		created() {
			(async function(self) {
				if (location.search.indexOf('type') != -1) {
					var res = await axios.get('http://localhost:5050/products_list/season', {
						params: {
							type: `${type}`,
							pno: `${pno}`
						}
					...
					var res = await axios.get('http://localhost:5050/products_list/search', {
						params: {
							keyword: `${keyword}`,
							pno: `${pno}`
						}
		...
		methods: {
			item_click: function(id) {
				window.open(`http://localhost:5050/products_women.html?wid=${id}`);
			},
